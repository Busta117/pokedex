...
class PokemonDetailViewController: BaseViewController {
    ...
    @IBOutlet fileprivate var movesButton: UIButton!
    ...
    func setupHeaderButtons() {
        ...
        movesButton.setTitleColor(color1, for: .normal)
        movesButton.backgroundColor = color2
        ...
        case .moves:
            movesButton.setTitleColor(color2, for: .normal)
            movesButton.backgroundColor = color1
        }
    }
    ...
    func registerCell() {
        ...
        tableView.register(UINib(nibName: "PokemonDetailMoveTableViewCell", bundle: nil), forCellReuseIdentifier: "PokemonDetailMoveTableViewCell")
    ...
    @IBAction fileprivate func movesAction(_ sender: Any) {
        tableMode = .moves
    }
}
...
extension PokemonDetailViewController {
    open func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        // let's move the white background
        ...
        // let's validate the color for the section header
        // print("\(contentOffsetY) - \(sectionHeaderView.frame.origin.y)")
...
extension PokemonDetailViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        case .moves:
            if let detail = viewModel.pokemonDetail.value {
                return detail.moves.count
            }
            return 0
        }
    }
    ...
    func moveCell(at indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "PokemonDetailMoveTableViewCell", for: indexPath) as! PokemonDetailMoveTableViewCell
        if let detail = viewModel.pokemonDetail.value, let move = Array(detail.moves)[safe: indexPath.row] {
...
extension PokemonDetailViewController {
    class func launch(withPokemon pokemon: Pokemon, viewModel: PokemonDetailViewModel? = nil) -> PokemonDetailViewController {
        let vc = UIStoryboard(name: "Main", bundle: nil).instantiateViewController(withIdentifier: "PokemonDetailViewController") as! PokemonDetailViewController
        if viewModel == nil {
            vc.viewModel = PokemonDetailViewModel(coordinator: MainCoordinator.shared) // default init, but injectable if needed
        }
